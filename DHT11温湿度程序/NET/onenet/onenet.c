...
#define PROID		"基于物联网的车内温度控制系统设计"

#define AUTH_INFO	"邹镇妙毕设"
			...
			if(result == 0)
			{
				...
				else
                {
                    ...
                    if(strstr(json_value->valuestring,"LED") != NULL)
                    {
                        ...
                        if(json_value->valueint)LED0 = 0;//关闭LED0
                        else LED0 = 1;//开灯
                    }
                    else
                    {
                        ...
                        alarm_is_free = 0;//上位机控制跟手动控制具有一样的优先级，优先于自动控制，因此清0
